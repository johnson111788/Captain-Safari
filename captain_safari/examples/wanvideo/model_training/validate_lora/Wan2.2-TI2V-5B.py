...
def process_videos_on_gpu(gpu_id, data_chunk, data_path, save_path):
    """
    This function will run on a single specified GPU, processing the data assigned to it.
    """
    ...
    # 2. Load the model into the specified GPU in the current process
    #    Each process needs its own model instance
    pipe = WanVideoPipeline.from_pretrained(
        torch_dtype=torch.bfloat16,
        device=device, # Load the model directly to the specified GPU
        model_configs=[
