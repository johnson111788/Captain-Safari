...
prompt = """
You are a perceptive visual assistant. Given a video input, generate a comprehensive caption describing the entire video. The caption should comprehensively reflect how the visual perspective changes over time and what is visible throughout.

* Describe the camera's movement smoothly across time, noting changes in elevation, direction, or rotation from start to end.
* Avoid phrases that refer to a visible drone (e.g., "the drone moves..."). Instead, use neutral or cinematic language to describe the movement, such as "the viewpoint ascends," "the camera glides," or "the aerial perspective circles."
* Include framing and camera angles (e.g., sweeping overhead, tilting down, tracking along a path).
* Vividly describe the environment — including terrain, structures, vegetation, water, weather, and lighting — in the order they appear, so the caption reflects temporal progression through the scene.

Avoid mentioning fine-grained details (e.g., faces, car models, written text), and do not speculate on anything not visually clear. Output only the final caption, with no tags or extra commentary.
"""
...
def process_videos_multiprocess(directory_path, output_file="captions.csv", max_workers=4):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_video = {executor.submit(generate_caption, vid): vid for vid in remaining_videos}
        ...
        with tqdm(total=len(remaining_videos), desc="Processing Videos", unit="vid", dynamic_ncols=True) as pbar:
            for future in as_completed(future_to_video):
                ...
                try:
                    ...
                    caption = f"ERROR: Future completed with an exception: {str(e)}"
                    logging.error(f"Error processing {os.path.basename(video_path)}: {caption}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scan a directory for videos, generate captions using the Gemini API, and save to a CSV file.")
    
    parser.add_argument("--directory", default='./data-slices', help="Directory containing video files to process.")
    parser.add_argument("--output", "-o", default="long_captions.csv", help="Output CSV file for captions (default: long_captions.csv).")
