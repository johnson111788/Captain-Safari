    ...
    if match_path is not None and match_path.exists():
        with h5py.File(str(match_path), "r", libver="latest") as fd:
            ...
            for i, j in pairs:
                ...
                for key in keys:
                    if key in fd:
                        # Check if matches0 and matching_scores0 exist and are valid
                        try:
                            grp = fd[key]
                            if "matches0" in grp and "matching_scores0" in grp:
                                m0 = grp["matches0"]
                                s0 = grp["matching_scores0"]
                                if (isinstance(m0, h5py.Dataset) and 
                                    isinstance(s0, h5py.Dataset) and 
                                    m0.shape == s0.shape):
                                    found = True
                                    break
                        except Exception:
