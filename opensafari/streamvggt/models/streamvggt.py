...
class StreamVGGT(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, feature_only=False, aux_feature=False):
        ...
        if feature_only:
            self.depth_head.feature_only = True
            self.point_head.feature_only = True
            self.track_head.feature_only = True
            
        if aux_feature:
            self.depth_head.feature_only = False
            self.point_head.feature_only = False
            self.track_head.feature_only = False
            self.point_head.aux_feature = True
    ...
    def inference(self, frames, query_points: torch.Tensor = None, past_key_values=None, export_memory=False):        
        ...
        for i, frame in enumerate(frames):
            ...
            # 如果需要导出内存，保存aggregated_tokens
            if export_memory:
                all_pose_enc[i] = aggregated_tokens[-1][:, :, 0]
                
                # 选择特定层的tokens并拼接
                selected_tokens = [aggregated_tokens[idx] for idx in [4, 11, 17, 23]]
                concat_tokens = torch.cat(selected_tokens, dim=1).detach().cpu()
                all_aggregated_tokens[i] = concat_tokens.detach()[:, :, :, -1024:]
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                pts3d_feature = None
                if self.point_head is not None:
                    if export_memory and hasattr(self.point_head, 'aux_feature') and self.point_head.aux_feature:   # 如果需要导出内存且point_head开启了aux_feature模式，保存特征
                        pts3d, pts3d_conf, pts3d_feature = self.point_head(
                            aggregated_tokens, images=images, patch_start_idx=patch_start_idx
                        )
                    else:
                        pts3d, pts3d_conf = self.point_head(
                    ...
                    # 如果需要导出内存且point_head开启了feature_only模式，保存特征
                    if export_memory:
                        # 假设在feature_only模式下，point_head返回特征而非最终输出
                        if pts3d_feature is not None:
                            all_pts3d_features[i] = pts3d_feature[:, 0].detach().cpu()
        ...
        # 如果需要导出内存，处理并返回内存数据
        if export_memory:
            # 拼接所有aggregated_tokens
            all_aggregated_tokens_cat = torch.cat(list(all_aggregated_tokens.values()), dim=0).cpu().numpy()
            del all_aggregated_tokens
            
            # 拼接所有pts3d_features（如果存在）
            if all_pts3d_features:
                all_pts3d_features_cat = torch.cat(list(all_pts3d_features.values()), dim=0).cpu().numpy()
                del all_pts3d_features
            else:
                all_pts3d_features_cat = None
                
            all_pose_enc_cat = torch.cat(list(all_pose_enc.values()), dim=0).cpu().numpy()
            del all_pose_enc
            
            return output, all_aggregated_tokens_cat, all_pts3d_features_cat, all_pose_enc_cat
    ...
    def export_memory(self, frames, query_points: torch.Tensor = None, past_key_values=None):        
        ...
        for i, frame in enumerate(frames):
            ...
            aggregated_tokens = torch.cat(aggregated_tokens, dim=1)
        ...
        return all_aggregated_tokens_cat, all_pts3d_features_cat, all_pose_enc_cat
