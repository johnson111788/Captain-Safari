    ...
    try:
        ...
        for s in range(0, len(video_batch), bs):
            ...
            try:
                ...
                with torch.no_grad():
                    tags = _generate_batch(model, processor, inputs)
                ...
                for vp in sub_paths:
                    try:
                        ...
                        tags.append(si_caps[0] if si_caps else "")
                    except Exception as ie:
                        tags.append(f"ERROR: {str(ie)}")
                    finally:
                        try:
                            del si_inputs
                        except Exception:
                ...
                tags = [f"ERROR: {str(e)}" for _ in sub_paths]

            # Stream results
            for vp, tag in zip(sub_paths, tags):
                try:
                    results_queue.put(("result", gpu_id, vp, tag))
                except Exception as e:
...
def process_videos_multi_gpu_stream(
    ...
    with open(output_file, "a", newline="", encoding="utf-8") as f, \
        ...
        while len(done_workers) < len(procs):
            ...
            tag = msg[0]
            if tag == "result":
                _, gid, video_path, tags = msg
                base = os.path.basename(video_path)
                if base not in written_basenames:
                    writer.writerow([base, tags])
                    f.flush()
                    os.fsync(f.fileno())
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        handlers=[logging.FileHandler("filter.log"), logging.StreamHandler()],
    )
    _mute_third_party_logs()

    parser = argparse.ArgumentParser(
        description="Filter out videos that do not meet drone footage standards using Qwen2.5-VL-7B model (multi-GPU, stream-writing, mini-batch)."
    )
    parser.add_argument("--directory", default="./data", help="Directory with video files.")
    parser.add_argument("--output", "-o", default="./filters.csv", help="Output CSV path.")
